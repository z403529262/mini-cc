...
- **MCP 工具一律放行**(`:83` `mcp__` 前缀直接 `return true`)——子 agent 也能用外部工具
...
mini-cc 只做"**独立 messages**"这一种隔离。真 CC 有两种：

- **独立上下文子 agent**(默认)：和 mini-cc 一样，全新 messages，子 agent 看不到主对话。
- **fork 子 agent**(实验)：反过来——**继承父的完整对话上下文 + 父已渲染的 system prompt 字节**(`runAgent.ts:508` 用 `override.systemPrompt`、`:370` 用 `forkContextMessages`)。为什么继承？为了 cache：子 agent 复用父的前缀就能命中父的 prompt cache。代价是要用 `buildChildMessage`(`forkSubagent.ts:171`) 给子 agent 立一套强纪律("你是 fork worker，别 spawn 子 agent、别废话、结构化汇报、≤500 词、开头必须是 'Scope:'")——因为它继承了父的 system，得用一段"压制指令"把它掰回 worker 心态。

mini-cc 的子 agent 反正是全新上下文 + 一段专门的 `SUBAGENT_SYSTEM`，天然就是 worker 心态，不需要这套压制。
